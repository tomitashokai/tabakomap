...
- 価格34件が null。裏が取れなかったものは埋めずに空けてある
- **`price_source` が null の行は参考価格。定価として表示してはいけない。**
  ...
  `price_as_of` が入る。2026-09-02 時点で定価30件 / 参考18件 / 価格なし34件
...
- `scripts/smoke.mjs` — 実 Chrome で主要操作を通しで踏む回帰確認（19項目）。別ターミナルで
  ...
  付ける→一覧→解除、登録モーダルの住所の自動入力を踏み直す。
  **お気に入りの項目だけ DB に書き込む**（通り抜ければ
...
価格が欠けているのではなく、行そのものを見直すこと。
「パーラメント・アクア・5」「バージニア・エス・メンソール・ワン」は PMJ の現行18銘柄に
含まれておらず終売の可能性が高い（**未確認**）。

**方針: 実在しないと確定した銘柄は行ごと消す。裏が取れていないだけの銘柄は残して参考価格にする。**
「参考」ラベルは"値段があやしい"を伝えるもので、"商品自体が無い"は伝えられない。
存在しない銘柄を残すと、探して店に行っても買えない。2026-09-02 に BAT 4件を
`20260902120000_drop_nonexistent_brands.sql` で削除した。

**消す前に「シリーズが無い」のか「サイズ表記が違うだけ」のかを分けること。**
BAT の紙巻き3件は最初「公式の定価表が見つからない」から存在しない扱いにしかけたが、
実際は全部実在した（`ラッキー・ストライク・エキスパートカット` は ・14 / ・10 の
サイズ展開があり 2026-06-01 に 450→470 円、`クール・マイルド` は
クール・マイルド・ボックスとして現行、`ケント・エス・シリーズ・ワン・100s` も実在）。
**シード名がサイズ抜きの総称になっているだけ。** 一方 `ネオ・テリック・タバコ` は
glo の終売銘柄54種の一覧にも無く、`ケント・ネオスティック・トゥルー・タバコ` は
glo pro 用の呼称「ネオスティック」と hyper 用の銘柄「トゥルー・タバコ」の合成名だった。
終売銘柄の一覧（`https://relazo.net/glopro_endofsale/`）は非公式だが網羅的で、
「そんな銘柄は無い」の判定に使える。
...
どの区ページにも出ず、詳細ページのパンくずにも区が出ない。そのため `AddSpotModal` は
現在地が確定した時点で `reverseGeocode()`（`src/lib/geocode.ts`）を呼び、住所欄の
下書きを埋める（投稿者が直せる。引けなければ空欄のまま手入力に任せる）。

**Mapbox の `full_address` をそのまま入れないこと。** 返るのは
「日本, 〒530-0005 大阪府大阪市北区中之島１丁目」の形で、取り込み済み542件の
「大阪市北区中之島1丁目」と書式が食い違う。`normalizeAddress()` で国名・郵便番号・
府名を落とす。府名は「大阪府大阪市」の並びのときだけ落とす（他府県まで一律に消すと、
大阪市外へ広げたときに市名だけ残ってどこの市か分からなくなる）。

逆ジオコーディングの失敗・タイムアウト・想定外の応答は全て null に倒し、例外を投げない。
住所は必須項目ではないので、ここで登録そのものを止めてはいけない。

### 区ページは DB 側で絞る。ただし ilike だけを答えにしない
`/areas/[ward]` は `fetchSpotsByWard()` で `address ilike '%大阪市○○区%'` を投げる
（以前は542件を毎回読んで JS で1区に絞っていた）。**ilike は前段の絞り込みでしかない。**
住所は投稿分が自由入力なので、別の区名が文中に出てくる行も拾いうる。区の確定は
`extractWard()` の完全一致で取り直すこと。2026-08-30 時点の542件では、24区すべてで
旧実装と件数が一致している。
...
### 「入れる人の制限」の判定は `isOpenToAll()` だけを見る
表示側で「`特になし` 以外は全部制限あり」と独自に判定してはいけない。
`SpotDetailSheet` がそう書いていたため、`加熱式専用` 3件と `地下街利用者のみ` 8件
（ホワイティうめだ、ドーチカ、なんばウォーク、NAMBAなんなん、あべちか）が、
地図には既定で出るのにピンを開くと「誰でも自由に立ち寄れる場所ではない」と
警告される状態だった。`/spots/[id]` と `/areas/[ward]` は `isOpenToAll` を見ていて
警告しないので、どこから開いたかで言うことが変わっていた。

条件は3つに分けて扱う。

- 入れる人の制限（`!isOpenToAll(spot)`）→ 琥珀の警告と ⚠️ バッジ
- 吸えるものの制限（`加熱式専用`）→ 「紙巻きたばこは吸えません」の補足。入場制限とは別に出す
- 通りがかれば該当する条件（`地下街利用者のみ`）→ 条件は見せるが警告にはしない
...
- 銘柄データの価格34件が null。**後述の落とし穴「価格は改定が続いている」を読んでから着手すること。**
  参考価格18件も公式の裏が取れていない
- **BAT の紙巻き3件（ケント・エス・シリーズ・ワン・100s / クール・マイルド /
  ラッキー・ストライク・エキスパートカット）は実在するが定価が未確認。**
  BAT は紙巻きの定価表を公式に出しておらず、小売店の情報しか見つからない。
  シード名もサイズ抜きの総称なので、実在名（`・14` `・ボックス` 等）に直すのが先
- JT・PMJ のシードにも同種の疑わしい銘柄が残っている（上記の落とし穴を参照）。
  BAT と同じ手順で1ブランドずつ潰すこと
